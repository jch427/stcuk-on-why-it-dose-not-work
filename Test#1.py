import PIL, random, sys from PIL
...
def create_square(border, draw, randColor, element, size):
    if (element == int(size/2)):    draw.rectangle(border, randColor)
    elif (len(listSym) == element+1):
        draw.rectangle(border,listSym.pop())
    else:
        listSym.append(randColor),    draw.rectangle(border, randColor)
...
for x in range(0, invaders):
    for y in range(0, invaders): topLeftX = x * invaderSize + padding / 2
